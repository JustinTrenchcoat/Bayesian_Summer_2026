...
run_metropolis_chain_2 <- function(n_samples, init_theta, proposal_sd = 0.5) {
  # initialization phase
  samples <- matrix(NA, nrow = n_samples, ncol = 2)
  current_theta <- init_theta
  samples[1, ] <- current_theta
  ...
  return(samples[, 2]) # Return only theta1 samples
}
...
theta1_matrix_4_1 <- matrix(NA, nrow = n_samples, ncol = n_chains_4)

theta1_matrix_512 <- matrix(NA, nrow = n_samples, ncol = n_chains_512)


init_point <- c(runif(1, -5, 5), runif(1, -5, 5)) 
...
for (j in 1:n_chains_4) {
  theta1_matrix_4_1[, j] <- run_metropolis_chain_2(n_samples, init_point, proposal_sd = 0.5)
}
# get squared error
...
se_matrix_4_1 <- matrix(NA, nrow = n_samples, ncol = n_chains_4)
for (j in 1:n_chains_4) {
  se_matrix_4_1[, j] <- cumsum(theta1_matrix_4_1[, j]) / (1:n_samples)
}
...
acf(theta1_matrix_4[,2], main = "ACF of Chain 2")
acf(theta1_matrix_4[,3], main = "ACF of Chain 3")
acf(theta1_matrix_4[,4], main = "ACF of Chain 4")
