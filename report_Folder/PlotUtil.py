...
# MVN experiment plotting functions:
# def MVN_MvR_color(c_df, n_df,pf_df,
#                   title, bound, threshold,
#                   num_subchains, color_choice):
#      fig, axes = plt.subplots(1,3, figsize=(25,10),dpi=150,
#                               sharex=True, sharey = True)
#      axes = axes.flatten()
#      dfs=[c_df, n_df, pf_df]
#      titles =["Constrained","Naive","PathFinder"]
#      if color_choice == "Dimension":
#          vmin = min(df["Dimension"].min() for df in dfs)
#          vmax = max(df["Dimension"].max() for df in dfs)
